...
struct parmetis_colorer_t : public colorer_t {
  ...
  std::set<size_t> color(const dcrs_t & dcrs) override {
    ...
    result = MPI_Alltoall(
      &send_cnts[0], 1, utils::mpi_typetraits__<idx_t>::type(), &recv_cnts[0],
      1, utils::mpi_typetraits__<idx_t>::type(), MPI_COMM_WORLD);
    ...
    for (size_t r(0); r < size; ++r) {
      if (recv_cnts[r]) {
        ...
        requests.push_back({});
        MPI_Irecv(
          &rbuffers[r][0], recv_cnts[r], utils::mpi_typetraits__<idx_t>::type(),
    ...
    for (size_t r(0); r < size; ++r) {
      if (send_cnts[r]) {
        ...
        MPI_Send(
          &sbuffers[r][0], send_cnts[r], utils::mpi_typetraits__<idx_t>::type(),
          r, 0, MPI_COMM_WORLD);
      } // if
    } // for
