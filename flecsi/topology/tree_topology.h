...
struct tree_geometry_u<T, 1> {
  ...
  //-----------------------------------------------------------------//
  //! Return true if point origin lies within the box specified by min/max
  //! point.
  //-----------------------------------------------------------------//
  static bool
  within_box(const point_t & origin, const point_t & min, const point_t & max) {
...
//-----------------------------------------------------------------//
enum class action : uint8_t { none = 0b00, refine = 0b01, coarsen = 0b10 };
...
public:
  ...
  void apply_in_radius(
    ...
    apply_(
      pool, sem, queue_depth, depth, b, size, f, geometry_t::intersects, center,
      radius);
  ...
  //-----------------------------------------------------------------//
  //! Visit and apply callable object f and args on all sub-branches of branch
  //! b.
  //-----------------------------------------------------------------//
  ...
  //-----------------------------------------------------------------//
  //! Visit and apply callable object f and args on all sub-entities of branch
  //! b.
  //-----------------------------------------------------------------//
  template<typename F, typename... ARGS>
...
private:
  ...
  apply_(branch_t * b, element_t size, EF && ef, BF && bf, ARGS &&... args) {
    ...
    for (size_t i = 0; i < branch_t::num_children; ++i) {
      ...
      if (bf(
            ci->coordinates(range_), size, scale_,
            std::forward<ARGS>(args)...)) {
        apply_(
      ...
      if (bf(
        ...
        if (depth == queue_depth) {
          ...
          auto f = [&, size, ci]() {
            ...
            find_(
              ci, size, branch_ents, std::forward<EF>(ef), std::forward<BF>(bf),
              std::forward<ARGS>(args)...);
