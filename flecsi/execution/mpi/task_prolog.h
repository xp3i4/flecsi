...
//     }
  ...
  handle(data_client_handle_u<T, PERMISSIONS> & h) {
    ...
    for (size_t i{0}; i < h.num_handle_entities; ++i) {
      ...
      storage->init_entities(
        ent.domain, ent.dim, ents, ids, ent.size, num_entities,
    ...
    for (size_t i{0}; i < h.num_handle_adjacencies; ++i) {
      ...
      storage->init_connectivity(
        adj.from_domain, adj.to_domain, adj.from_dim, adj.to_dim,
    ...
    for (size_t i{0}; i < h.num_index_subspaces; ++i) {
      ...
      storage->init_index_subspace(
        iss.index_space, iss.index_subspace, iss.domain, iss.dim,
        reinterpret_cast<utils::id_t *>(iss.indices_buf), num_indices, _read);
    }
  ...
  handle(data_client_handle_u<T, PERMISSIONS> & h) {
    ...
    for (size_t i{0}; i < h.num_handle_entities; ++i) {
      ...
      storage->init_entities(
        ent.index_space, ent.index_space2, ents, 0, active_ents, 0,
        migrate_ents, 0, ent.size, _read);
    }
  }
